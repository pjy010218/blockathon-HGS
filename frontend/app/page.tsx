...
function DataView({ wallet, walletError, submission, onConnect, onSubmit }: DataViewProps) {
  ...
  useEffect(() => {
    if (submission.phase === "idle") return;
    bannerRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
  }, [submission.phase, submission.message]);

  function fillCoordsFromSite(value: string) {
    const needle = value.trim().toLowerCase();
    if (!needle) return;
    const match = stations.find((station) => station.name.trim().toLowerCase() === needle)
      ?? stations.find((station) => station.id.toLowerCase() === needle);
    if (!match || !latitudeRef.current || !longitudeRef.current) return;
    latitudeRef.current.value = String(match.latitude);
    longitudeRef.current.value = String(match.longitude);
  }
  ...
  return <section className="content-view view-enter" aria-labelledby="data-title">
    ...
    {submission.phase !== "idle" && <div ref={bannerRef} className={`submission-banner ${submission.phase}`} role={submission.phase === "error" ? "alert" : "status"} tabIndex={-1}>{submission.phase === "error" ? <span className="banner-symbol">!</span> : <Icon name="check" />}<div><b>{submission.phase === "error" ? "Submission stopped" : submission.phase === "unmatched" ? "Accepted, not map-visible" : submission.phase === "submitted" ? "Record accepted" : "Submission in progress"}</b><span>{submission.message}</span>{submission.contentHash && <code>{submission.contentHash}</code>}</div></div>}
